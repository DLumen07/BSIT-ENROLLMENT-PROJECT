...
import { Subject as ScheduleSubject } from '@/app/admin/dashboard/schedule/[blockId]/page';
...
export default function AdminInstructorScheduleView() {
    ...
    const { instructors, schedules, academicYear, semester, semesterOptions } = adminData;
    ...
    if (!instructor) {
        return (
            <main className="flex-1 p-4 sm:p-6">
                <h1 className="text-2xl font-bold tracking-tight">Instructor Not Found</h1>
    ...
    for (const blockName in schedules) {
        const blockSchedule = schedules[blockName];
        const subjectsInBlock = blockSchedule.filter(sub => sub.instructor === instructor.name);
    ...
    return (
        <main className="flex-1 p-4 sm:p-6 space-y-6">
            <div className="space-y-0.5">
                <h1 className="text-2xl font-bold tracking-tight">Instructor Schedule</h1>
                <p className="text-muted-foreground">
                    Viewing schedule for <span className="font-semibold text-foreground">{instructor.name}</span> | A.Y. {academicYear}, {semesterLabel}
                </p>
            </div>
                ...
                <CardContent className="p-4 overflow-x-auto">
                     <div className="grid grid-cols-[4rem_repeat(6,1fr)] min-w-[800px]">
                        ...
                        <div className="col-start-2 col-span-6 row-start-2 relative grid grid-cols-6" style={{ height: `${timeSlots.length * HOUR_HEIGHT_REM}rem`}}>
                            ...
                            {instructorSchedule.map(subject => {
